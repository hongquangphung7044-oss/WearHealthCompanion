...
| 修复基线构建 | **Build #51** / tag `build-51` / commit `a293b77` |
| Build #51 Actions | `https://github.com/hongquangphung7044-oss/WearHealthCompanion/actions/runs/29305818846`（协议测试、双 Release APK、Artifact、Release 全部成功） |
| Build #51 Release | `build-51`，含手机与手表 ECG APK，tag 精确指向修复提交；后续应先通过 API 查询更新构建 |
...
| 实机验证状态 | Build #51/#52 实机确认手表入口可见，但 128-bit UUID 硬件过滤扫描未发现手机广播。下一构建改为无 UUID 硬件过滤、软件校验广播，并显示分层扫描诊断；完整链路仍需国行 One UI Watch 8 实测 |
...
| #51/#52 | ⚠️ 构建成功，扫描实测失败 | 手表入口正常，但使用 128-bit Service UUID 的系统 ScanFilter 在国行 Watch 8 上未返回手机广播 |
| 下一构建 | 验证中 | 空系统过滤器扫描 + 软件解析/匹配 128-bit UUID；同步期间常亮；高功率手机广播；分层扫描诊断 |
...
Build #51/#52 的实机错误为“未发现手机 BLE 同步器”，说明流程停在扫描阶段，尚未进入 GATT 连接或 Key 加密读取。部分 Android/Samsung 蓝牙控制器对自定义 128-bit Service UUID 的硬件 `ScanFilter` 会静默漏报。因此下一构建改为：

```text
使用空的系统 ScanFilter 接收广播
→ 在应用内先查 ScanRecord.serviceUuids
→ 再解析原始 AD type 0x06 / 0x07 的 128-bit UUID
→ 仅 UUID 匹配后检查 BOND_BONDED 并连接
```

这不会放宽设备授权：非本项目 UUID 不连接，未配对设备不连接，Key characteristic 仍要求 encrypted read。超时 UI 会区分“零广播”“看到周边但没看到手机”“看到手机但未配对”。Key 获取和 ECG 上传期间保持手表屏幕常亮，避免 Android 暂停无约束扫描。
