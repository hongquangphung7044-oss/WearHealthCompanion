...
| 实机验证状态 | Build #54 实机确认手机 Key 已成功传到手表，证明 bonded 直连、GATT Service 发现和加密读取已打通；ECG 上传仍失败。下一构建改为先确认 Service 再协商 MTU、失败自动重连重传一次，并明确显示 BLE 待机/已连接/接收/Room/ACK 状态。完整 ECG 回传和 ACK 仍需国行 One UI Watch 8 实测 |
...
| 下一构建 | 验证中 | Data Layer 提交后等待匹配的 Room ACK，8 秒未确认自动转 BLE；BLE 先发现并确认 Service 再协商 MTU；传输中断自动重连并幂等重传一次；手机明确显示 BLE 连接及 ACK 实际发送状态 |
