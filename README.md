...
| 最近确认成功构建 | **Build #50** / tag `build-50` / commit `8eda8ba`（但内置 fallback 导致“无 Key”入口被隐藏，不作为首次配置验证版） |
| 当前验证构建 | 下一次 main Actions：验证真实空白安装、BLE 诊断、协议测试及双 APK 强校验 |
| Build #50 Actions | `https://github.com/hongquangphung7044-oss/WearHealthCompanion/actions/runs/29304271688` |
...
| 实机验证状态 | 手机 Build #50 已确认 BLE 广播就绪；Build #50 手表因编译时 Key fallback 隐藏 BLE 获取按钮。下一构建修复入口并增加 bondState/GATT 诊断，完整 Key 获取与 ECG 回传仍需国行 One UI Watch 8 实测 |
...
| 下一构建 | 验证中 | 移除 APK 内置 Key；无 Key 显示“获取”，已有 Key 仍显示“更新”；增加 bondState/GATT 诊断和协议单测 |
...
### P0：确认修复 Build（Build #50 之后的下一个 main Build）
