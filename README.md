...
| 自动发布项目 | 最新成功状态 |
|---|---|
| CI 构建 | **Build #59** / tag `build-59` / source commit `a48c5a2` |
| Actions | `https://github.com/hongquangphung7044-oss/WearHealthCompanion/actions/runs/29314127834`（协议测试、双 Release APK、Artifact、Release 全部成功） |
| Release | `https://github.com/hongquangphung7044-oss/WearHealthCompanion/releases/tag/build-59` |
| APK | `WearHealthCompanion-mobile-v1.0.59-code59.apk`；`WearHealthCompanion-watch-v1.0.59-code59-ecg.apk` |
| 更新时间 | 2026-07-14T07:26:45Z |
...
| 实机验证状态 | Build #54 实机确认手机 Key 已成功传到手表；Build #57 实机进一步确认 BLE 已连接、Service/MTU/ACK 通道完成且手机成功接收 63,748 字节 payload 的 BEGIN，但手表在 BEGIN 后提交下一 CHUNK 时被 Samsung GATT 队列拒绝。Build #58 实机表现为点击传送后手表 App 闪退且手机未收到 ECG；代码审查定位到 Android 14 会将首次请求的 MTU 提升为 517，而 CHUNK 公式生成 514 字节 attribute value，新 Android 13+ 写 API 会对超过 512 字节的 value 抛出异常。下一构建将完整 CHUNK 帧限制为 512 字节并把平台异常转换为可见错误。完整 ECG 回传与 ACK 仍需国行 One UI Watch 8 实测 |
...
| 下一构建 | 验证中 | 保留严格串行写和 busy 原帧重试；将完整 CHUNK 帧限制为 512 字节，并捕获平台参数/权限异常转为手表可见诊断，避免进程直接退出 |
