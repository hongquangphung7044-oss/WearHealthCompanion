...
| 自动发布项目 | 最新成功状态 |
|---|---|
| CI 构建 | **Build #61** / tag `build-61` / source commit `ecf8dd9` |
| Actions | `https://github.com/hongquangphung7044-oss/WearHealthCompanion/actions/runs/29320000994`（协议测试、双 Release APK、Artifact、Release 全部成功） |
| Release | `https://github.com/hongquangphung7044-oss/WearHealthCompanion/releases/tag/build-61` |
| APK | `WearHealthCompanion-mobile-v1.0.61-code61.apk`；`WearHealthCompanion-watch-v1.0.61-code61-ecg.apk` |
| 更新时间 | 2026-07-14T09:06:46Z |
...
| 默认分支 | `main` |
| 当前功能 | BLE Key 获取入口对“无 Key / 已有 Key”均可见；手表主页始终保留手工输入 API Key 入口；手机设置页可清除缓存（API Key + ECG 历史）并重启 BLE；ECG 波形改用固定时间/电压比例（接近 25 mm/s、10 mm/mV），不再自适应拉伸；Release APK 不再内置 HeartVoice Key |
