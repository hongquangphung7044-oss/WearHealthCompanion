...
| 实机验证状态 | 手机 Build #50 已确认 BLE 广播就绪；Build #51 已修复手表入口并发布。完整 Key 获取、ECG 分片回传、Room 幂等和 ACK 仍需使用 Build #51 两端 APK 在国行 One UI Watch 8 实测 |
...
| #51 | ✅ 成功，待国行实机验证 | 移除 APK 内置 Key；无 Key显示“获取”、已有 Key显示“更新”；增加 bondState/GATT 诊断、协议单测及双 APK 强校验 |
