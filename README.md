...
| 实机验证状态 | Build #53 实机确认第三方 BLE 扫描回调为零；下一构建增加 `neverForLocation`、优先直连系统 bonded 设备，并修复 ECG 采样回调与实时波形读取的并发竞争。完整 Key 获取、ECG 回传和 ACK 仍需国行 One UI Watch 8 实测 |
...
| 下一构建 | 验证中 | `neverForLocation` + bonded 设备直连优先、扫描回退；Key 获取严格 25 秒、ECG 上传严格 45 秒；修复 ECG 采样列表并发读取导致的无详情“采集失败” |
...
Build #51/#52 的错误为“未发现手机 BLE 同步器”；Build #53 改用空过滤器后，实机进一步确认“未收到任何 BLE 广播”，说明 One UI Watch 8 未向本 App 交付扫描回调。部分 Android/Samsung 蓝牙控制器不仅会对自定义 128-bit Service UUID 的硬件 `ScanFilter` 静默漏报，还可能受 Nearby Devices AppOps 限制。因此下一构建同时采用两条路径：
