...
| 自动发布项目 | 最新成功状态 |
|---|---|
| CI 构建 | **Build #115** / tag `build-115` / source commit `5929e7b` |
| Actions | `https://github.com/hongquangphung7044-oss/WearHealthCompanion/actions/runs/29478978866`（协议测试、双 Release APK、Artifact、Release 全部成功） |
| Release | `https://github.com/hongquangphung7044-oss/WearHealthCompanion/releases/tag/build-115` |
| APK | `WearHealthCompanion-mobile-v1.0.115-code115.apk`；`WearHealthCompanion-watch-v1.0.115-code115-ecg.apk` |
| 更新时间 | 2026-07-16T07:17:27Z |
...
├── data/DeepSeekSettings.kt           # DS Key + 思考档位存储
├── data/TavilySettings.kt             # Tavily Key 存储（DS Max 档联网检索）
├── ble/BleApiKeyFetcher.kt            # 手表无历史也可从手机读取 Key
├── ble/BleDsSettingsFetcher.kt        # 从手机读取 DS 设置
├── ble/BleTavilySettingsFetcher.kt    # 从手机读取 Tavily Key
├── ble/BleEcgUploader.kt              # 手表扫描、连接、分片上传、等待 Room ACK
├── service/WatchWearableListenerService.kt  # 处理 /api_key /ds_settings /tavily_settings
└── ui/
    ├── HealthViewModel.kt             # 测量、Key、DS/Tavily 设置、Data Layer/BLE 选择、历史
    ├── HealthMonitorScreen.kt         # 手表主页（含 DS/Tavily Key 输入框）
...
│   ├── MobileDeepSeekSettings.kt      # 手机侧 DS 设置
│   └── MobileTavilySettings.kt        # 手机侧 Tavily Key
├── service/
│   ├── BleSyncServer.kt               # 手机 GATT Server / advertiser（含 DS/Tavily 特征值）
│   ├── BleSyncRuntime.kt              # 进程级唯一 BLE Server 所有权
│   ├── BleSyncForegroundService.kt    # connectedDevice 前台服务/常驻通知
│   ├── DataLayerManager.kt            # 含 sendDsSettings/sendTavilySettings 下发
│   └── PhoneWearableListenerService.kt
├── ui/MobileViewModel.kt              # saveAndSendDsSettings/saveAndSendTavilySettings
├── ui/SettingsScreen.kt               # 含 DS / Tavily 卡片
├── ui/HistoryListScreen.kt
├── ui/MeasurementDetailScreen.kt      # parseAiReport 解析 DS JSON 报告
├── ui/MobileEcgChart.kt
└── pdf/PdfExporter.kt                 # PDF 含 Fridericia QTc

shared/src/main/java/com/wearhealth/companion/shared/
├── DataLayerPaths.kt                  # /api_key /ds_settings /tavily_settings
├── EcgMeasurementTransfer.kt          # 跨端模型 + EcgBinaryCodec
├── MeasurementSerializer.kt           # DataMap / JSON 元数据
├── BleSyncProtocol.kt                 # UUID（含 DS_SETTINGS / TAVILY_SETTINGS）、帧、CRC
├── ApiKeyValidator.kt                 # 三层清洗 NUL + 控制字符
├── DeepSeekApiClient.kt               # DS API + Max 档 Tavily 检索注入 + 循证提示词
├── TavilyApiClient.kt                 # Tavily 搜索 + 结果清洗 + 进程内缓存
├── JsonCleaner.kt                     # 剥离 Markdown + 全角转半角
└── EcgFeatureExtractor.kt             # 本地 DSP（Pan-Tompkins R 波 / HRV 双通路 / 间期估测）
```
...
- 交互波形支持拖动和双指缩放；
- 支持手工输入 HeartVoice Key、DS Key、Tavily Key，以及 Data Layer 下发 / BLE 主动读取（三种 Key 各走独立 GATT 特征值与 DataLayer 路径）；
...
```text
手机输入
→ MobileApiKeyStore / MobileDeepSeekSettings / MobileTavilySettings（SharedPreferences）
→ 已配对手表通过加密 GATT read 主动获取（三种 Key 各自独立特征值/路径）
→ ApiKeyManager / DeepSeekSettings / TavilySettings（手表 SharedPreferences）
→ HeartVoiceApiClient / DeepSeekApiClient / TavilyApiClient
```

| Key | GATT 特征值 | DataLayer 路径 | 用途 |
|---|---|---|---|
| HeartVoice Key | `API_KEY_UUID` (9a4b7d02) | `/api_key` | HeartVoice 专业 API |
| DeepSeek Key | `DS_SETTINGS_UUID` (9a4b7d04) | `/ds_settings` | DS V4-Flash 分析（Key + 思考档位） |
| Tavily Key | `TAVILY_SETTINGS_UUID` (9a4b7d05) | `/tavily_settings` | DS Max 档联网检索 |

Data Layer 路径仍保留，GMS 可用时手机也会尝试提交三种 Key。

### 重要安全事项

- 不得把 GitHub PAT、HeartVoice Key、DeepSeek Key、Tavily Key、keystore 或 Samsung AAR 写入 README、源码、Git remote、提交或日志；
- 当前手机和手表三种 Key 都是普通 SharedPreferences，**不是加密存储**；
...
当前 `estimateIntervals` 用阈值交叉法（QRS）+ T 波峰定位法（QT），均为工程实现非临床金标准。QRS 阈值交叉法 10% 相对专业 API 偏宽 0-10ms；QT 用 T 波峰而非 T 波终点（临床金标准为 Lepeschkin 切线法，Postema 2014），系统性偏短约 50-100ms。**若需更准的间期数值**，可移植 NeuroKit2 的 `ecg_delineate`（连续小波变换，同时给 P/QRS/T 波边界）：
...
当前算法阈值均已有文献依据（见 `EcgFeatureExtractor.kt` 注释），但**未经大规模临床数据验证**。关键经验值（即使有文献方向支持）需在真实数据上回归：

- CV>0.20 判房颤（文献无统一切点，各研究用 RCV+SVM 分类）
- SD1/SD2 比值切点（文献用机器学习，无统一比值切点）
- 短长配对 0.8/1.2 倍均值（方向对，倍数是经验值）
- 配对数≥3 提示早搏（经验值，无文献标准）

所有结论均为"筛查辅助，非诊断金标准"。若需提升精度，建议采集真实房颤/早搏/正常样本，用 ROC 分析校准阈值。
