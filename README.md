...
| 自动发布项目 | 最新成功状态 |
|---|---|
| CI 构建 | **Build #57** / tag `build-57` / source commit `67a0359` |
| Actions | `https://github.com/hongquangphung7044-oss/WearHealthCompanion/actions/runs/29311390816`（协议测试、双 Release APK、Artifact、Release 全部成功） |
| Release | `https://github.com/hongquangphung7044-oss/WearHealthCompanion/releases/tag/build-57` |
| APK | `WearHealthCompanion-mobile-v1.0.57-code57.apk`；`WearHealthCompanion-watch-v1.0.57-code57-ecg.apk` |
| 更新时间 | 2026-07-14T06:35:11Z |
...
| 实机验证状态 | Build #54 实机确认手机 Key 已成功传到手表；Build #57 实机进一步确认 BLE 已连接、Service/MTU/ACK 通道完成且手机成功接收 63,748 字节 payload 的 BEGIN，但手表在 BEGIN 后提交下一 CHUNK 时被 Samsung GATT 队列拒绝。下一构建改为回调退出后延迟串行写、成功回调后才推进序号、GATT busy 原帧重试。完整 ECG 回传与 ACK 仍需国行 One UI Watch 8 实测 |
...
| 下一构建 | 验证中 | GATT 写严格串行：回调退出后延迟 12 ms 再写；成功回调后才推进帧号；busy 时原帧延迟重试，不跳序号 |
