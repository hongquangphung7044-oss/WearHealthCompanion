...
2. 间期估测的已知系统偏差（判断时需留余量，并在报告注明"以专业 API/临床心电图为准"）：
   - QRS 宽度：阈值交叉法，相对专业 API 偏小约 10-20ms（判 >120ms 时需更保守）
...
- 节律（基于 RR 变异系数 + Poincaré 散点形态 + 短-长配对，这是节律判断的主要依据）:
  ...
  * 短-长 RR 配对数≥2 且 Poincaré 复杂形 → 提示早搏（PAC/PVC 需形态判断，本设备无法区分，统一报告为"早搏可能"）
...
- 间期判断（基于本地估测，参考范围，注意系统偏差）:
  ...
  * QRS 宽度: 80-120ms 正常；本地估测偏小，判 >100ms 时就需警惕束支阻滞
  * QTc: 男性<440ms 女性<460ms 正常；本地估测偏大，判 >480ms 时才需警惕长 QT
- 信号质量:
...
【输出格式 - 严格 JSON，无任何 JSON 外内容。所有字段必须在顶层，禁止嵌套对象（DS 历史上嵌套输出易产生语法错误）】
{
  "平均心率": 70,
  "心率范围": "65-75 bpm",
  "心率稳定性": "...",
