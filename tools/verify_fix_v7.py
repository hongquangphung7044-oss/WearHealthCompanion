...
基于 v4 验证有效的部分 + v5/v6 验证有效的改进点：
- v4 有效：QRS SNR 自适应（偏差 9.6ms ✓）、切线法无范围限制、T 峰窗口 R+100~450ms、SNR 阈值 2.5
- v5/v6 有效：基线用 R 峰前 PR 段、Q 点用 QRS 宽度、local_hr 用 RR 中位数（抗漏检）

放弃 v5/v6 失败的部分：
- v5 失败：切线法限范围 30-150ms（124550 +149ms）、SNR 阈值降到 1.5
- v6 失败：高质量周期选择（周期数太少，中位数平均不稳定）、RR 异常排除（周期数更少）

置信度分级（v6 思路保留）：
- 高置信（T_SNR>3）：实测
- 中置信（1.5<T_SNR≤3）：贝叶斯 = 实测×0.5 + Bazett先验×0.5
- 低置信（T_SNR≤1.5）：Bazett 先验（用 RR 中位数心率）
"""
import os
import glob
...
def estimate_qt_final(ecg, r_peaks, avg_hr, qrs_width_ms, sr=SR):
    """QT：信号平均 + 置信度分级（最终版）
    保留 v4 有效：所有周期均值平均、切线法无范围限制、T 峰窗口 R+100~450ms、SNR 阈值 2.5
    改进点：基线用 R 峰前 PR 段、Q 点用 QRS 宽度、置信度分级（贝叶斯）
    """
    n = len(ecg)
    if len(r_peaks) < 5:
        return (0, 0, 0, "无周期", _qt_prior_bazett(avg_hr))
    ...
    if min_len < sr * 350 // 1000:
        return (0, 0, 0, "无周期", _qt_prior_bazett(avg_hr))
    # 均值平均（v4 思路，v6 验证中位数平均在周期数少时不稳定）
    n_cycles = len(cycles)
    avg = [0.0] * min_len
    for s, e in cycles:
        for i in range(min_len):
            avg[i] += ecg[s + i]
    avg = [x / n_cycles for x in avg]
    ...
    r_amp = abs(r_val)
    if r_amp < 20:
        return (0, 0, 0, "R波低", _qt_prior_bazett(avg_hr))
    ...
    if t_end_in_win <= t_start_in_win + 10:
        return (0, 0, 0, "T窗窄", _qt_prior_bazett(avg_hr))
    # 平滑
    sw = 7
    smoothed = []
    ...
    for i in range(1, len(smoothed) - 1):
        ...
        if is_peak and abs(smoothed[i]) > max_dev:
            ...
            t_idx = i
    if t_idx < 0:
        return (0, 0, 0, "无T峰", _qt_prior_bazett(avg_hr))
    # T 波 SNR
    ...
    # T 终 = T 峰 + 固定间隔（100ms）
    # 根因：切线法/阈值法都依赖 T 终定位，但 T 终在低 SNR 下不可靠
    # T 峰-T终间隔生理学相对稳定（80-120ms，正常 T 波降支时长）
    # 依据：Surawicz 2009 (Chou's ECG in Modern Medicine)；T 波降支约 100ms
    t_peak_to_end_ms = 100
    t_end_idx = t_idx + t_peak_to_end_ms * sr // 1000
    ...
    qt_prior = _qt_prior_bazett(avg_hr)
    # 置信度分级
    if t_snr > 3.0 and 250 <= qt_measured <= 500:
        return (qt_measured, t_snr, n_cycles, "高置信", qt_measured)
    elif t_snr > 1.5 and 250 <= qt_measured <= 500:
        qt_bayes = int(qt_measured * 0.5 + qt_prior * 0.5)
        return (qt_bayes, t_snr, n_cycles, "中置信", qt_measured)
    else:
        return (qt_prior, t_snr, n_cycles, "低置信", qt_measured)


def main():
    files = sorted(glob.glob(os.path.join(SAMPLES_DIR, "ECG_diagnostic_*.txt")))
    print("=" * 150)
    print("修复版算法验证报告 v7（最终版：v4有效部分 + 中位数心率 + PR基线 + Q点用QRS宽度 + 置信度分级）")
    print("=" * 150)
    print(f"{'文件':<32} {'HV_HR':<6} {'local_HR':<9} {'HV_QRS':<7} {'新QRS':<7} {'QRS差':<7} {'HV_QT':<6} {'新QT':<6} {'QT差':<6} {'实测QT':<7} {'先验QT':<7} {'T_SNR':<7} {'N周期':<6} {'置信度':<8} {'短RR':<5}")
    print("-" * 150)
    sum_qrs_diff = 0
    sum_qt_diff = 0
    n_samples = 0
    for f in files:
        ...
        # QT
        new_qt, t_snr, n_cycles, confidence, qt_measured = estimate_qt_final(ecg, peaks, local_hr, new_qrs)
        qt_prior = _qt_prior_bazett(local_hr)
        # 短 RR
        short_rr = 0
        ...
        n_samples += 1
        print(f"{os.path.basename(f):<32} {hv_hr:<6} {local_hr:<9} {hv_qrs:<7} {new_qrs:<7} {qrs_diff:<+7} {hv_qt:<6} {new_qt:<6} {qt_diff:<+6} {qt_measured:<7} {qt_prior:<7} {t_snr:<7.2f} {n_cycles:<6} {confidence:<8} {short_rr:<5}")
    print()
    if n_samples:
