...
- QT：信号平均法 v3
  改进点（v5）：
  1. 基线估计：只用 R 峰前 PR 段（窗口起点 R-80ms 到 R-50ms 的 30ms 均值）
     根因：v4 用两端均值，但窗口末端在 T 波下降支，把 T 波振幅当基线 → R/T 振幅被压缩
  2. 中位数平均：用周期中位数代替均值，抗异常周期（漏检导致的合并周期）
  3. 切线法搜索范围：限制在 T 峰后 30-150ms（T 波降支正常生理范围）
     根因：v4 无范围限制，切线法可能选 T 峰后第一个噪声陡降段（如 124550 偏早 67ms）
  4. T 波 SNR 阈值：2.5 → 1.5（信号平均后 T 波 SNR 通常 1.7-4.6，2.5 太严）
  5. Q 点定位：用 QRS 估测的 Q 点位置（R 峰前 QRS 宽度一半），不用 5% 阈值
     根因：v4 用 5% 阈值交叉法，平均波形 R 波振幅大时 Q 点找不到 → 默认 R 位置 → QT 偏小
"""
import os
import glob
...
def estimate_qt_signal_averaged_v3(ecg, r_peaks, avg_hr, qrs_width_ms, sr=SR):
    """QT：信号平均法 v3
    改进点（v5）：
    1. 基线估计：只用 R 峰前 PR 段（窗口起点 R-80ms 到 R-50ms 的 30ms 均值）
    2. 中位数平均：用周期中位数代替均值，抗异常周期
    3. 切线法搜索范围：限制在 T 峰后 30-150ms（T 波降支正常生理范围）
    4. T 波 SNR 阈值：2.5 → 1.5
    5. Q 点定位：用 QRS 宽度（R 峰前 QRS 宽度一半），不用 5% 阈值
    """
    n = len(ecg)
    if len(r_peaks) < 5:
    ...
    for pos in range(min_len):
        ...
        avg[pos] = vals[n_cycles // 2]  # 中位数
    # 改进1：基线只用 R 峰前 PR 段（窗口起点 R-80ms 到 R-50ms 的 30ms 均值）
    # 根因：v4 用两端均值，但窗口末端在 T 波下降支，把 T 波振幅当基线
    # PR 段在 R 峰前 50-80ms（P 波之后 R 波之前），是真正的等电位线
    bl_start = 0  # 窗口起点 = R-80ms
    bl_end = min(sr * 30 // 1000, win_before - sr * 10 // 1000)  # R-80ms 到 R-50ms
    ...
    if r_amp < 20:
        return _qt_prior_bazett(avg_hr)
    # 改进5：Q 点用 QRS 宽度（R 峰前 QRS 宽度一半）
    # 依据：R 峰通常在 QRS 的后 1/3（Q-R 间期约 QRS 宽度的 30-40%）
    # QRS 宽度来自 SNR 自适应法估测（已验证有效，平均偏差 9.6ms）
    # 如果 QRS 宽度未知，回退到 R 峰前 40ms（QRS 宽度 100ms 的一半）
    qrs_half_ms = (qrs_width_ms // 2) if qrs_width_ms > 0 else 50
    qrs_half_ms = max(20, min(80, qrs_half_ms))  # 限制在 20-80ms
    ...
    # 改进4：T 波 SNR 阈值 2.5 → 1.5
    if t_snr < 1.5:
        return _qt_prior_bazett(avg_hr)
    # 改进3：切线法（Lepeschkin 法）找 T 波终点
    # 搜索范围限制在 T 峰后 30-150ms（T 波降支正常生理范围）
    # 依据：T 波降支通常 80-120ms，30-150ms 覆盖正常范围
    # 根因：v4 无范围限制，切线法可能选 T 峰后第一个噪声陡降段
    is_positive_t = smoothed[t_idx] > 0
    t_end_idx = t_idx
    search_start = t_idx + sr * 30 // 1000  # T 峰后 30ms
    search_end = min(len(smoothed) - 1, t_idx + sr * 150 // 1000)  # T 峰后 150ms
    if is_positive_t:
...
def main():
    ...
    print("=" * 130)
    print("修复版算法验证报告 v5（基线改进+中位数平均+切线限范围+Q点用QRS宽度）")
    print("=" * 130)
    ...
    for f in files:
        ...
        for r in peaks:
            ...
            if res:
                ...
                if 40 <= qms <= 200:
                    qrs_vals.append(qms)
                    qrs_q_points.append(res[1])
        new_qrs = sum(qrs_vals) // len(qrs_vals) if qrs_vals else 0
        # QT (信号平均法 v3)
        qt_res = estimate_qt_signal_averaged_v3(ecg, peaks, local_hr, new_qrs)
        if qt_res:
