...
/**
 * 通过 BLE 从手机同步器拉取 DeepSeek 设置（API Key + 模型 + 思考强度 + 年龄/性别）。
 *
 * 与 [BleApiKeyFetcher] 同思路（国行无 GMS 设备也可用）：
 * 1. 优先直连 [BlePhoneDeviceStore] 缓存的手机 MAC（上次成功连接的手机）
 * 2. 回退到扫描广播
 * 3. 连接成功后读取 [BleSyncProtocol.DS_SETTINGS_UUID] 特征值（JSON）
 *
 * 返回的 JSON 由手机端 [com.wearhealth.companion.mobile.service.BleSyncServer.buildDsSettingsJson] 构造，
 * 字段名与 [com.wearhealth.companion.shared.DataLayerPaths] 的 DS 设置 key 一致。
 */
class BleDsSettingsFetcher(private val context: Context) {
    ...
    /** 拉取 DS 设置 JSON。成功返回 JSON 字符串。 */
    fun fetch(done: (Result<String>) -> Unit) {
        if (callback != null) {
            done(Result.failure(IllegalStateException("DS 设置获取已在进行中")))
            return
        }
        if (!hasPermissions()) {
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            if (finished) return
            if (gatt !== this@BleDsSettingsFetcher.gatt) {
            ...
            this@BleDsSettingsFetcher.gatt = gatt
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
            if (finished || gatt !== this@BleDsSettingsFetcher.gatt) return
            val service = if (status == BluetoothGatt.GATT_SUCCESS) {
                gatt.getService(BleSyncProtocol.SERVICE_UUID)
            } else null
            val characteristic = service?.getCharacteristic(BleSyncProtocol.DS_SETTINGS_UUID)
            if (characteristic == null) {
                if (tryingBondedCandidate) connectNextBondedCandidate()
                else finish(Result.failure(IllegalStateException("手机同步器没有 DS 设置特征值")))
                return
            }
            matchedService = true
            handler.removeCallbacks(candidateTimeout)
            if (!gatt.readCharacteristic(characteristic)) {
                finish(Result.failure(IllegalStateException("无法请求读取 DS 设置")))
            }
        }
        ...
        override fun onCharacteristicRead(
            ...
            if (finished || gatt !== this@BleDsSettingsFetcher.gatt ||
                characteristic.uuid != BleSyncProtocol.DS_SETTINGS_UUID) return
            if (status != BluetoothGatt.GATT_SUCCESS) {
                finish(Result.failure(IllegalStateException("手机拒绝读取 DS 设置（GATT $status）")))
                return
            }
            val bytes = characteristic.value ?: ByteArray(0)
            val json = String(bytes, Charsets.UTF_8).trim()
            if (json.isEmpty() || json == "{}") {
                finish(Result.failure(IllegalStateException("手机端尚未配置 DeepSeek 设置")))
                return
            }
            // 校验是合法 JSON 再返回
            try {
                ...
                finish(Result.failure(IllegalStateException("DS 设置 JSON 解析失败: ${e.message}")))
                return
            }
